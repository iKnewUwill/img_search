...
from app.cbir import utils
# from networkx.readwrite import gpickle # 新版本已弃用
...
class VocabularyTree(object):
    ...
    def save(self, path=None):
        ...
        # store graph
        # nx.write_gpickle(self.graph, os.path.join(path, "graph.pickle"))   #最新版本已经弃用
        # gpickle.write_gpickle(self.graph, os.path.join(path, "graph.pickle"))
        with open(os.path.join(path, "graph.pickle"), "wb") as f:
    ...
    def load(self, path=None):
        """
        Load a previously saved vocabulary tree from disk.
        Args:
            path (str): The directory path where the graph and nodes are stored.
        """
        if path is None:
            path = "data"

        # load the graph
        # self.graph = nx.read_gpickle(os.path.join(path, "graph.pickle")) # 最新版本已经弃用
        # gpickle.read_gpickle(os.path.join(path, "graph.pickle"))
        with open(os.path.join(path, "graph.pickle"), "rb") as f:
        ...
        # Initialize tree and other necessary attributes after loading
        self.tree = {node: [child for child in self.graph.successors(node)] 
                    for node in self.graph.nodes}
        self._current_index = max(self.graph.nodes) + 1 if self.graph.nodes else 0
        self._propagated = set()
        return True
